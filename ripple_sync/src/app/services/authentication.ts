...
export class Authentication {
  ...
  registerState = signal<RegisterResponseState>({
    status: null,
    message: null,
  ...
  login(credentials: LoginRequest): Observable<AuthenticationResponse> {
    return this.http
      .post<AuthenticationResponse>(`${environment.apiUrl}/authentication/login`, credentials, {
        observe: 'response',
      })
      .pipe(
        tap({
  ...
  register(register: RegisterRequest): void {
    ...
    this.http
      .post<ProblemDetails>(`${environment.apiUrl}/authentication/register`, register, {
        observe: 'response',
      })
      .pipe(
        tap({
          ...
          error: (error) => {
            ...
            if (error.status === 400 && problemDetails?.validationErrors) {
              this.registerState.update((state) => ({
                ...
                validationErrors: problemDetails.validationErrors || null,
              }));
            } else if (error.status >= 400 && error.status < 500 && problemDetails) {
              this.registerState.update((state) => ({
                ...state,
                status: 'error',
                message: 'An error occurred. ' + problemDetails.detail,
              }));
            } else {
              this.registerState.update((state) => ({
                ...state,
  ...
  deleteAccount(): void {
    this.registerState.set({ status: 'loading', message: null, validationErrors: null });
    this.http
      .delete<HttpResponseBase>(`${environment.apiUrl}/authentication/user`, {
        observe: 'response',
      })
      .pipe(
        tap({
          ...
          error: (error) => {
            alert("Error happended while trying to delete user try again later")
          },
        })
      )
      .subscribe();
